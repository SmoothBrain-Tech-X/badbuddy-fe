...
import { bookingService, Booking } from '@/services';
...
// config/bookings.ts
export const bookingStatus: BookingStatusConfig[] = [
...
const BookingManagement = () => {
    ...
    const calculateStats = (bookingData: Booking[]) => {
        const newStats = bookingData.reduce((acc, booking) => ({
            ...acc,
            total: acc.total + 1,
    ...
    const getStatusConfig = (status: string): BookingStatusConfig => {
        return bookingStatus.find(s => s.value === status) ||
            { label: status, value: status, color: 'gray' };
    };
        ...
        return (
            <Paper p="md" radius="md" withBorder>
                ...
                <Group justify="space-between" mb="md">
                    <div>
                        <Text fw={600} size="lg" mb={4}>{booking.court_name}</Text>
                        <Group gap="xs">
            ...
            <Container size="xl">
                <Grid gutter="xl">
                    <Grid.Col span={{ base: 12, md: 6 }}>
                        <Stack gap="lg">
                            <div>
                                <Text size="xl" fw={500}>Booking Management</Text>
                                <Title order={1}>Your Court Bookings</Title>
                            </div>
                            <Text size="lg">
                                View and manage all your court bookings and reservations.
                            </Text>
                        </Stack>
                    </Grid.Col>
                </Grid>
    ...
    return (
        <Box bg="gray.0" mih="100vh"> {/* Added minimum height */}
            <HeaderSection />

            <Container size="xl" py="xl" h="100%"> {/* Added height */}
                <Grid gutter="lg" style={{ minHeight: 'calc(100vh - 200px)' }}> {/* Adjusted for header height */}
                    {/* Stats Section */}
                    <Grid.Col span={12}>
                        <Grid>
                    ...
                    <Grid.Col span={12} style={{ flex: 1 }}>
                        <Card shadow="sm" radius="md" withBorder h="100%">
                            <Stack h="100%">
                                <Tabs value={activeTab} onChange={(value) => setActiveTab(value || 'upcoming')} style={{ flex: 1 }}>
                                    <Tabs.List>
                                        <Tabs.Tab
                                            value="upcoming"
                                        ...
                                        <Stack>
                                            {filterBookings('upcoming').length > 0 ? (
                                                ...
                                                <EmptyState
                                                    ...
                                                    onAction={() => {/* Add your navigation logic */ }}
                                                />
                                            )}
                                        </Stack>
                                    </Tabs.Panel>
