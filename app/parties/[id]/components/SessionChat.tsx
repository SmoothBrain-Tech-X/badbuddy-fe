...
// Message Component
const MessageCard: React.FC<{ message: APIMessage; currentUserId: string }> = ({
  message,
  currentUserId,
}) => {
  ...
  return (
    <Group align="flex-start" wrap="nowrap" justify={isOwn ? 'flex-end' : 'flex-start'}>
      {!isOwn && (
        <Avatar src={message.autor.avatar_url || '/api/placeholder/32/32'} radius="xl" size="md" />
      )}
...
// Main Chat Component
const SessionChat: React.FC<SessionChatProps> = ({ sessionId, isDisabled }) => {
  const theme = useMantineTheme();
  ...
  const fetchInitialMessages = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${CONFIG.API_BASE_URL}/api/chats/session/${sessionId}/messages`,
        {
          headers: {
  ...
  return (
    <Card radius="lg" p={0} mt={8}>
      <Stack gap="lg">
        <ScrollArea h={400} viewportRef={scrollAreaRef} offsetScrollbars>
          <Stack gap="md" p="md" justify="flex-end" style={{ minHeight: '100%' }}>
            {messages.map((msg) => (
              <MessageCard key={msg.id} message={msg} currentUserId={user?.id || ''} />
            ))}
