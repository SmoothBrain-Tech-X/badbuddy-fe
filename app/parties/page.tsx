...
import { Session, sessionService, User } from '@/services';
...
const PartyListPage: React.FC = () => {
  ...
  const fetchParties = useCallback(async () => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to load parties', {
        id: 'loading-parties',
      });
    }
  ...
  const handleJoinLeave = async (partyId: string) => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to update party status', {
        id: 'join-leave',
      });
    }
  };
      ...
      <Container size="xl" py="xl">
          ...
          <Group mb="xl">
            ...
            {level && (
              <Badge
                variant="light"
                rightSection={
                  <ActionIcon variant="transparent" size="sm" onClick={() => setLevel(null)}>
                    ×
                  </ActionIcon>
                }
              >
            ...
            {timeFilter && (
              <Badge
                variant="light"
                rightSection={
                  <ActionIcon variant="transparent" size="sm" onClick={() => setTimeFilter(null)}>
                    ×
                  </ActionIcon>
                }
              >
        ...
        {parties.length > 0 && (
          <>
            <Grid>
              {parties.map((party) => (
                <Grid.Col key={party.id} span={{ base: 12, md: 6, lg: 4 }}>
                  <PartyCard party={party} onJoinLeave={() => handleJoinLeave(party.id)} />
                </Grid.Col>
              ))}
            </Grid>
            ...
            {/* Create Party Button */}
            <Box pos="fixed" bottom={40} right={40} style={{ zIndex: 1000 }}>
              <Button
                size="lg"
                radius="xl"
